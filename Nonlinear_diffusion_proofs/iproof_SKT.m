function [rmin,rmax]=iproof_SKT(u,Abar,w11,w12,w21,w22,para,nu)
...
u_ext = intval(zeros(2*(2*N-1),1));
u_ext(1:N) = u1;
u_ext(2*N-1+(1:N)) = u2;
F_ext = F_DF_SKT(u_ext,para);

InvLap = spdiags([0,-1./(pi^2*(1:(4*N-4)).^2)]',0,4*N-3,4*N-3);
...
disp(['Z1_finite = ',num2str(Z1_finite.sup)])
...
Z1_tail = max( xi_nu(1:2*N-1) * abs(e_ext-convo(w11,DPhi11,2*N-1)-convo(w12,DPhi21,2*N-1)) ...
               ...
               + 1/(pi*N)^2 * ( norm_w_DR(1,1) + norm_w_DR(2,1) ), ...
               ...
               + 1/(pi*N)^2 * ( norm_w_DR(1,2) + norm_w_DR(2,2) ) );
disp(['Z1_tail = ',num2str(Z1_tail.sup)])
...
normAblock_tail = 1/(pi*N)^2 * norm_w;
normAblock = max(normAblock_finite, normAblock_tail);

Lap = -pi^2*diag((0:N-1).^2);
...
Delta = (1-Z1)^2-2*Y*Z2;
if Z1.sup<1 && Delta.inf>0
    rmin = sup( (1-Z1-sqrt(Delta)) / Z2 );
    rmax = inf( (1-Z1) / Z2 );
